...
namespace XmlRpc {
  ...
  void XmlRpcValue::assertTypeOrInvalid(Type t)
  {
    ...
    {
      roslite_debug_printf("type error");
      roslite_debug_break();
    }
  }
  ...
  void XmlRpcValue::assertArray(int size) const
  {
    if (_type != TypeArray)
    {
      roslite_debug_printf("type error: expected an array");
      roslite_debug_break();
    }
    else if (int(_value.asArray->size()) < size)
    {
      roslite_debug_printf("range error: array index too large");
      roslite_debug_break();
    }
  }
  ...
  void XmlRpcValue::assertStruct()
  {
    ...
    {
      roslite_debug_printf("type error: expected a struct");
      roslite_debug_break();
    }
  }
...
#endif // ROSLITE_TARGET_CLUSTER_ID != 0
